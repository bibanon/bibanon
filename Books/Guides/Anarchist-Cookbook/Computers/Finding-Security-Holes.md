...
1. security objects: items to be protected. ie: a users file.
2. control objects: items that protect security objects. ie: a i-node
3. mutual objects  : objects in both classes. ie: the password file
...
_Any sugestions on how to implement such a DB?_
...
I suggest everyone obtain, and install agrep from:
...
Another tool worth producing is a program to generate
...
Posted to USENET (what FTP archive?) (descrip?)

### PAPERS

There are several papers that discuss methods of finding flaws, and
present test suites.
...
8. Holes can often be found in code that:
  1. is ported to a new environment.
  2. receives unexpected input.
  3. interacts with other local software.
  4. accesses system files like passwd, L.sys, etc.
  5. reads input from a publicly writable file/directory.
  6. diagnostic programs which are typically not user-proofed.
...
11. Look for seldom used, or unusual functions or commands - read backwards.
   ...
   option to login ... right, as most login.c's I've seen have:
